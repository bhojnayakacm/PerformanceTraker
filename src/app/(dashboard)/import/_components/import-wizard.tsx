...
const MODULES: ModuleConfig[] = [
  {
    ...
    category: "master",
    instructions: [
      "Matched by name — names are globally unique, so a CSV with name-only rows updates existing employees in place.",
      "emp_id is required only when creating a brand-new employee. Leave it blank to update by name.",
      "If emp_id is provided, it takes precedence as the upsert key — useful for correcting a name typo on an existing record.",
      "location, state, date_of_joining, and reporting_manager_name are optional.",
      "date_of_joining accepts dd/mm/yyyy, dd-mm-yyyy, dd.mm.yyyy, or ISO YYYY-MM-DD.",
      "reporting_manager_name must match an existing Tier-1 employee exactly (no chains: managers cannot themselves report to anyone).",
    ],
    columns: [
      { name: "name", required: true },
      { name: "emp_id", required: false },
      { name: "location", required: false },
      { name: "state", required: false },
      { name: "date_of_joining", required: false },
      { name: "reporting_manager_name", required: false },
    ],
  },
  {
    value: "targets",
